...
import swaydb.data.{SwayMap, SwaySet, request}
...
object SwayDB extends LazyLogging {
  ...
  def createSwayDBTable(core: CoreAPI)(implicit ordering: Ordering[Slice[Byte]]): SwayDBTable = {
    val raw = new SwayDBRaw(core)
    new SwayDBTable(raw)
  }
  ...
  def persistent[K, V](dir: Path,
    ...
    CoreAPI(
      ...
      core =>
        SwayMap[K, V](createSwayDBTable(core), rootTable)
    }
  ...
  def persistentSet[T](dir: Path,
    ...
    CoreAPI(
      ...
      core =>
        SwaySet[T](createSwayDBTable(core), rootTable)
    }
  }
